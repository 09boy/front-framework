...
export function parseSmartOption({ cli, args }: SmartOption, defaultData: SmartConfigOption, packageData?: PackageData): SmartTaskOption {
  ...
  if (cli === 'server') {
    return getServerTaskOption({ ...args, htmlPath: args?.htmlPath || defaultData.buildDir }, defaultData.port, defaultData.host);
  }
...
function getServerTaskOption(args?: SmartCliArgs, port?: number, host?: string): SmartTaskOption {
  return {
    cli: 'server',
    serverOption: {
      port: args?.port || port || 4001,
      host: args?.host || host || '127.0.0.1',
