...
async function App(): Promise<void> {
  ...
  const smartCli: SmartCliType[] = isSTProject ? developProjectCli : createProjectCli;
  const smartCommandValue = await SmartCli(smartCli);
  const smartTaskValue = await getSmartConfigureData(isSTProject, smartCommandValue);
  smartTaskValue && await Smart(smartTaskValue);
}

App().finally(() => {console.log('');});
